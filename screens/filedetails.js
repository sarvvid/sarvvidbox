           ...
           <Card1 dark = {darkTheme} onPress={()=>{
               RNFS.mkdir(`/storage/emulated/0/Pictures/SarvvidBox`).then(res=>{
                ...
                Toast.show({
                    type: 'success',
                    text1: 'Downloading...',
                    text2: `${item.filename} is downloading....`
                  });
              }).catch(err=>{
                console.log(err);
              })
            RNFetchBlob.config({
                ...
                addAndroidDownloads : {
                    useDownloadManager : true,
                      ...
                      mediaScannable: true,
                      title : item.filename,
                      path  : `${RNFetchBlob.fs.dirs.DownloadDir}/${item.filename}_enc`,
                  }
            }).fetch('GET',`http://103.155.73.35:3000/cat?filehash=${item.filename}&IMEI=${imei}&ping=${settedping}`,{ 'Cache-Control': 'no-store',"Authtoken":auth}).progress((received,total)=>{
                console.log('progress',received/total);
            }).then(async(resp)=>{
                const pri = await AsyncStorage.getItem("mypri")
                const path = await RNVirgilCrypto.decryptFile(
                    normalizeFilePath(RNFetchBlob.fs.dirs.DownloadDir+`/${item.filename}_enc`),  
                    `/storage/emulated/0/Pictures/SarvvidBox/${item.filename}`,
                    pri,
                  ) 
               ...
               <Section1>
                   <Image source = {navigation.route.params.image}/>
                   <View style = {{marginLeft:10}}>
                       <Title dark  = {darkTheme}>{item.filename.substring("0",item.filename.lastIndexOf("."))}</Title>
                       <SectionDetails dark = {darkTheme}>type: {item.filename.substring(item.filename.lastIndexOf("."))} size:{item.filesize}</SectionDetails>
                   </View>
               </Section1>
               <Section2>
                   <Image source = {navigation.route.params.arrow}/>
               </Section2>
           </Card1>
        </View>
    }
